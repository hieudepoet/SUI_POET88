...
// BeepLancer Agent Tools
import { scoutAgentsTool } from './tools/scoutAgents.js';
import { hireAgentTool } from './tools/hireAgent.js';
import { signSuiTransactionTool } from './tools/signSuiTransaction.js';
import { submitDeliveryTool } from './tools/submitDelivery.js';
import { verifyDeliveryTool } from './tools/verifyDelivery.js';
 ...
 * - Beep Payment: Payment processing tools
 * - Agent Lifecycle: Scouting, hiring, delivery management
 * - Blockchain: SUI transaction signing
 */
const tools: MCPToolRegistry = {
  ...
  // BeepLancer Agent Tools
  scoutAgents: scoutAgentsTool,
  hireAgent: hireAgentTool,
  signSuiTransaction: signSuiTransactionTool,
  submitDelivery: submitDeliveryTool,
  verifyDelivery: verifyDeliveryTool,
};
...
async function main() {
  ...
  // Use the extracted handler for the /mcp endpoint
  const mcpHandler = createMcpHttpHandler({
    logger: {
      debug: (msg, obj) => console.log(`[DEBUG] ${msg}`, obj || ''),
      log: (msg, obj) => console.log(`[INFO] ${msg}`, obj || ''),
  ...
  app.get('/health', (req, res) => {
    res.json({ status: 'ok', service: 'beep-mcp-server' });
  });

  app.listen(port, () => {
    console.log(`BEEP MCP Server running on http://localhost:${port}/mcp`);
